...
//AUDIO CHANNEL
#define PIN1_CD4051       A0
#define PIN2_CD4051       A1
...
#define TIME_START_CALL       2000    //2000 ms
...
pinMode(SIM900_STATE, 0);

pinMode(PSW,1);

//enable channels

pinMode(PIN1_CD4051, 1);
pinMode(PIN2_CD4051, 1);

digitalWrite(PIN1_CD4051,0);
digitalWrite(PIN2_CD4051,0);
...
//add error control
Serial.println("init done, wait for phone get signal");
bool wait = true;
uint8_t bootCmd = 0;
char readCmd;
while(wait){

while(phone.available() > 0){ //if we read +CREG ==> phone receive network
readCmd = phone.read();
...
delay(10000); //once it found something wait 5 more seconds
...
if(stateCall == NO_CALL){
  if(!digitalRead(SIM900_STATE)){ //someone is calling you
    //stateCall
    Serial.println("ring");
   ringTimeOut = millis();
   if(!setRing){
...
if(csFlag == true){//if read enabled
  ...
  timerCall = millis(); // reset timer to prevent call
}
...
if(AP_NUMBER > 0){ //reset number if user make a wrong number
  for(uint8_t i = 0; i < 20; i++){
  number[i] = 0;
}
  AP_NUMBER = 0;
...
Serial.println("back to 0");
  ...
  //if there a call, close it
  if(stateCall == CALLING){
    closeCall();
stateCall = NO_CALL;
Serial.println("NO_CALL");
digitalWrite(PIN1_CD4051,0); //enable Arduino audio channel
/*digitalWrite(PIN1_SIM900,0);
digitalWrite(PIN2_SIM900,0);
digitalWrite(PIN1_ARDUINO,1);
digitalWrite(PIN2_ARDUINO,1);*/
...
if(!digitalRead(SWB) && stateCall == NO_CALL){
  swFlag = false;
}

 //if we make a call
if(!digitalRead(SWB) && stateCall == NO_CALL && millis() - timerCall >= TIME_START_CALL && AP_NUMBER > 0){
...
digitalWrite(PIN1_CD4051,1);//enable SIM900 audio channel
setRing = false;
}



if(!digitalRead(SWB) && swFlag && stateCall == RECEIVE_CALL){//do something
  swFlag = false;
...
if(stateCall == RECEIVE_CALL){
  if(digitalRead(SIM900_STATE)){
    stateCall = NO_CALL;
  Serial.println("Go in no call");
  }
  if(millis() - ringTimeOut >= 4000){
    counterRing = 0;
    ringTimeOut = millis();
  }
